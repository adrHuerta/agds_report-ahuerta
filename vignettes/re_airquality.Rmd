...
In this exercise, we explore the seasonal variability of ozone concentration in New York during the months of May to September 1973, using the built-in R dataset airquality. The dataset comes from the New York State Department of Conservation and contains daily air quality measurements collected in New York City. The target variable is ozone, which we investigate in relation to meteorological variables such as solar radiation, wind speed, and maximum air temperature. These variables provide the basis to address the following question: 
...
The airquality dataset consists of daily measurements from May to September 1973, and include different variables (Table 1). Due to these variables are in unit that not belong to SI units, we transformed them into SI units to enhance clarity and interpretation. We also checked the dataset for missing values. Ozone and Solar.R contain some missing entries, which were handled appropriately (complete case analysis) during the analysis to avoid distortion of statistical results. 
...
To underastad the data at first, we summarize basic metrics for the cleaned dataset: mean, min, max and standard deviation for all variables (Table 2).
...
To explore how ozone and related meteorological variables vary over the summer months, we created boxplots grouped by month (from May to September) for Ozone and meteorological variables. These plots help reveal seasonal trends, variability, and potential outliers.
...
To better understand how each meteorological variable influences Ozone concentration, we created scatterplots with linear regression lines. These plots help visualize the direction and strength of the relationships between Ozone and Temp, Wind, and Solar.R.
